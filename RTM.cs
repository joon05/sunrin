...
public class RTM : MonoBehaviour//빨강팀 바법사
{
    //능력치 세팅
    public AudioSource audioSource;
    public static int rtm_life = 1;
    ...
    void Start()
    {
        ...
        this.transform.position = new Vector3(-3, (float)-3, -1);//매 시작 위치
    }
    ...
    private void OnMouseDown()//클릭되었을때
    {
        ...
        if (MainControl.Instance.anyc == false)//아무도 클릭되어 있지 않다면
        {
            if (MainControl.Instance.turn == "red") //만약 턴이 레드팀이라면
            {
                ...
                MainControl.Instance.su = "RTM";//선택된 유닛 RTM
